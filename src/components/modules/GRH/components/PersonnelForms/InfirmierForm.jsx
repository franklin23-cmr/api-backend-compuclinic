...
import { submitInfirmier, updateInfirmier } from '../../network/grh.network';

export const InfirmierForm = ({
  isLoading,
  setIsLoading,
  dataInForm,
  submitButtonText,
    ...
    <Form
      ...
      onFinish={async (data) => {
        ...
        if (dataInForm) {
          ...
          updateInfirmier(dataInForm.id, data)
            .then((result) => {
              setIsLoading?.(false);
              console.log(result);
              if (result?.success || result?.id) {
                notification.success({
                  message: 'Succès',
                  description: 'Infirmière modifiée avec succès',
                });
                history.push('/grh/personnel', { tabIndex: '4' });
              } else {
                notification.error({
          ...
          submitInfirmier(dataToPost)
            .then((result) => {
              ...
              if (result?.success) {
                notification.success({
                  message: 'Succès',
                  description: 'Infirmière a été créée avec succès',
                });
                history.push('/grh/personnel', { tabIndex: '4' });
              } else {
                notification.error({
