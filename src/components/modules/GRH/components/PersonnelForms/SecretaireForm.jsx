...
import { submitSecretaire, updateSecretaire } from '../../network/grh.network';

export const SecretaireForm = ({
  isLoading,
  setIsLoading,
  dataInForm,
  submitButtonText,
    ...
    <Form
      ...
      onFinish={async (data) => {
        ...
        if (dataInForm) {
          ...
          updateSecretaire(dataInForm.id, data)
            .then((result) => {
              setIsLoading?.(false);
              if (result?.success || result?.id) {
                notification.success({
                  message: 'Succès',
                  description: 'La secretaire a été modifiée avec succès',
                });
                history.push('/grh/personnel', { tabIndex: '2' });
              } else {
                notification.error({
          ...
          submitSecretaire(dataToPost)
            .then((result) => {
              ...
              if (result?.success) {
                notification.success({
                  message: 'Succès',
                  description: 'La secretaire a été crée avec succès',
                });
                history.push('/grh/personnel', { tabIndex: '2' });
              } else {
                notification.error({
